...
import com.aenlly.rcc.utils.service.SearchLibraryAdd;
...
public class SearchServiceImpl implements ISearchService {
    /** 垃圾分类库的服务 */
    @Resource
    private ISearchLibraryService searchLibraryService;
    /** http请求对象 */
    @Resource
    ...
    /** 插入数据库未拥有的数据对象 */
    @Resource
    private SearchLibraryAdd searchLibraryAdd;
     ...
     */
    @Override
    public Collection<SearchLibrary> searchText(String name, String userId, SearchTypeEnum searchTypeEnum) {
        // 增加搜索记录
    ...
    public String transferVoiceApi(String data, Long dataLen) {
        ...
        //                 + "}");
        // 唯一请求 ID
        String usrAudioKey = String.valueOf(UUID.randomUUID()
                                                .getLeastSignificantBits());
        // 请求参数
        String reqData = String.format(VOICE_TX_STRING.getValue(), usrAudioKey);
        log.warn("reqData请求参数:{}", reqData);
        // 实例化一个请求对象,每个接口都会对应一个request对象
        SentenceRecognitionRequest req = SentenceRecognitionRequest.fromJsonString(reqData, SentenceRecognitionRequest.class);
