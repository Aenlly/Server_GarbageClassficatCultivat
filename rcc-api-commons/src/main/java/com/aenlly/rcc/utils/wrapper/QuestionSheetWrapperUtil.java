...
public class QuestionSheetWrapperUtil {
    ...
    private static final String SUBMIT_TIME_FILED = "submit_time";
    ...
    /**
     ...
     @return 查询对象
     */
    public static Wrapper<PaperTables> queryListPage(Long belongId, String startTime, String endTime) {
        QueryWrapper<PaperTables> wrapper = new QueryWrapper<>();
        wrapper.eq("belong_questionnaire_id", belongId)
               .orderByDesc(SUBMIT_TIME_FILED);
        if (StringUtils.isNotBlank(startTime)) {
            wrapper.ge(SUBMIT_TIME_FILED, startTime);
        }
        if (StringUtils.isNotBlank(endTime)) {
            wrapper.le(SUBMIT_TIME_FILED, endTime);
        }
        return wrapper;
    }
