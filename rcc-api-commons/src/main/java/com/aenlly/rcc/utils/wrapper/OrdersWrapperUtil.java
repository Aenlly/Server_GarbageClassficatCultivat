...
public class OrdersWrapperUtil {
    ...
    /**
     ...
     @return 查询对象
     */
    public static Wrapper<OrderVo> queryListPage(QueryOrderTypeEnum queryType, String text, OrderStateEnum state) {
        QueryWrapper<OrderVo> wrapper = new QueryWrapper<>();
        wrapper.like(queryType.getValue(), text)
               .eq("o.delete_flag", 0)
               .orderByDesc("o." + UPDATE_TIME_FILED);
        if (state != null) {
    ...
    public static Wrapper<Orders> getWriteOff(Long id) {
        ...
        wrapper.set("state", OrderStateEnum.WRITEN_OFF)
               .eq("order_id", id);
        return wrapper;
    }
}
