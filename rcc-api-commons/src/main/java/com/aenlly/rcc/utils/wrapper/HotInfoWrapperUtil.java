...
public class HotInfoWrapperUtil {
   ...
   * @return 查询对象
   */
  public static Wrapper<HotInfoVo> queryListPage(
      QueryHotInfoTypeEnum queryType, String text, HotInfoStateEnum state) {
    QueryWrapper<HotInfoVo> wrapper = new QueryWrapper<>();
    wrapper.like(queryType.getValue(), text).eq("delete_flag", 0);
    if (state != null) {
  ...
  public static Wrapper<HotInfo> updateStateById(Long id, HotInfoStateEnum state) {
    ...
    wrapper.set("hot_info_state", state).eq("hot_info_id", id);
    return wrapper;
  }
}
