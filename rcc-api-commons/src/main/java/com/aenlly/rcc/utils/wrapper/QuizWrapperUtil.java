...
public class QuizWrapperUtil {
    ...
    /**
     ...
     @return 查询对象
     */
    public static Wrapper<PaperTables> getPaperTablesExist(String userId, Long questionnaireId, Integer weekNumber) {
        QueryWrapper<PaperTables> wrapper = new QueryWrapper<>();
        wrapper.eq("user_id", userId)
               .eq("belong_questionnaire_id", questionnaireId);
    ...
    public static Wrapper<QuestionnaireTopics> getTopicByRandomBatchIndex(String randomBatchIndex) {
        ...
        wrapper.eq("random_batch_index", randomBatchIndex)
               .eq("state", 0);
        return wrapper;
    }
    ...
    /**
     ...
     @return 查询对象
     */
    public static Wrapper<SubjectTable> getSubjectTableLimitTen(Integer databankId, Integer topicCount) {
        QueryWrapper<SubjectTable> wrapper = new QueryWrapper<>();
        wrapper.eq("databank_id", databankId)
               .last("ORDER BY  RAND() LIMIT " + topicCount);
        return wrapper;
    }
    ...
    public static Wrapper<OptionsTable> getOptionsTableListBySubjectId(Long id) {
        ...
        wrapper.select("id", "option_name", "belong_topic_id")
               .eq("belong_topic_id", id);
        return wrapper;
    }
    ...
    public static Wrapper<PaperTables> getPaperTablesOne(String userId, String randomIndex) {
        QueryWrapper<PaperTables> wrapper = new QueryWrapper<>();
        wrapper.eq("user_id", userId)
               .eq("random_batch_index", randomIndex);
        return wrapper;
    }
    ...
    /**
     ...
     @return 查询对象
     */
    public static Wrapper<QuestionnaireTopics> getQuestionnaireTopicBy(Long topicId, String randomIndex) {
        QueryWrapper<QuestionnaireTopics> wrapper = new QueryWrapper<>();
        wrapper.eq("belong_topic_id", topicId)
               .eq("random_batch_index", randomIndex);
        return wrapper;
    }
    ...
    public static Wrapper<QuestionnaireTopics> getQuestionnaireTopicAnswer(String randomIndex) {
        QueryWrapper<QuestionnaireTopics> wrapper = new QueryWrapper<>();
        wrapper.eq("random_batch_index", randomIndex)
               .eq("state", TopicStateEnum.UN_ANSWERED);
        return wrapper;
    }
    ...
    public static Wrapper<OptionsTable> getOptionsTableCorrect(Long topicId) {
        QueryWrapper<OptionsTable> wrapper = new QueryWrapper<>();
        wrapper.eq("belong_topic_id", topicId)
               .eq("correctly_or_not", CorrectlyOrNotEnum.CPRRECTLY);
        return wrapper;
    }
    ...
    public static Wrapper<PaperTables> getPaperTablesLog(String userId, String questionnaireName) {
        QueryWrapper<PaperTables> wrapper = new QueryWrapper<>();
        wrapper.select("id", "submit_time", "total_score")
               .eq("user_id", userId)
